 ...
 * 4.Dynamically allocated obejcts are destroyed when the delete operator is applied
 * to a pointer to the obejct.
 * 5.Temporary objects are destroyed at the end of the full expression in which
 * the temporary was created.
 *
 * Note:
 * The destructor is not run when a reference or a pointer to an object goes out of
 * scope.
 * 
 ...
 * A destructor body executes in addition to the memberwise destruction that takes
 * place as part of destroying an obejct.
 * 
 * 
 * 
 ...
 * 
 * Some classes have work that needs to be done to copy or assign objects but has
 * no need for the destructor.
 * 
 * 
 * 
 * Using = default
 * 
 * We can explicitly ask the compiler to generate the synthesized versions of the copy-control
 * members by defining them as = default.
 * 
 * Note:
 ...
 * that is declared but may not be used in any other way.
 * 
 ...
 * the same object as the right-hand operand. This behavior is unlikely to be desired.)
 * 
 * 4.So does the synthesized default constructor. If the class has a member
 * with a deleted or inaccessible destructor or has a reference or const member that 
 * does not have an
 * 
 * 
