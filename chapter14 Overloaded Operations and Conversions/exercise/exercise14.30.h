...
std::string *StrBlobPtr::operator->() { return &operator*(); }
...
class ConstStrBlobPtr {
  ...
  private:
	std::shared_ptr<const std::vector<std::string>> check() const;
...
inline const std::string *ConstStrBlobPtr::operator->() { return &operator*(); }

std::shared_ptr<const std::vector<std::string>> ConstStrBlobPtr::check() const {
	auto ret = wptr.lock();
	if (!ret)
		throw std::runtime_error("unbound StrBlobPtr");
	if (curr >= ret->size() || curr < 0)
